...
The generated execution policy is intended for scientific and bioinformatic work. Scientific analysis and visualization requests, including CSV-backed heatmap demos, must use an appropriate scientific library stack (for example, Pandas/NumPy with Matplotlib or Seaborn) and must be built and run in Docker through the nested Docker daemon. Do not choose Python-standard-library-only code as a workaround for dependency installation.
